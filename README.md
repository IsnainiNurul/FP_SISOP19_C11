...
* Berikut ini adalah library yang diperlukan untuk membuat daemon :
```
#include<sys/types.h>
#include<sys/stat.h>
#include<stdio.h>
...
```
* Berikut ini untuk mendeklarasi variable global:
```
char kal1[100];
char kal2[100];
char kal3[100];
...
* Berikut ini untuk mendeklarasi variable untuk keperluan thread:
```
pthread_t tid1, tid2;
```

* Berikut ini adalah fungsi thread untuk memfilter waktu dan mengeksekusi crontab :
```
void* cron(void *arg)
{
...
* Berikut ini adalah fungsi untuk mereverse kalimat pada perintah :
```
...
* Berikut ini adalah template untuk pembuatan daemon :
```
  pid_t pid, sid;
  pid = fork();
  if (pid < 0) 
...
* Berikut ini adalah fungsi Main Program untuk menjalankan daemon :
```
while(1) 
  {
    struct tm *infowakta;
...
```
* Berikut adalah untuk mengakhiri pembuatan daemon (bagian dari template) :
```
fclose(baca);
  }
  exit(EXIT_SUCCESS);
